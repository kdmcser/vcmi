	...
	const auto & runMainLoop = []()
	{
		try
		{
			if (ENGINE)
			{
				...
				std::string content = R"({英雄无敌3增强版VCMI团队声明}
...
3.如果你已付费，可以将此界面截图并到12315（www.12315.cn）举报商家的涉嫌侵权行为。若商家在销售过程中存在欺诈行为（如冒充正版、虚假宣传），你可以依据《消费者权益保护法》第五十五条主张退款并要求三倍价款赔偿（增加赔偿的金额不足五百元的，按五百元计算）；即使不构成欺诈，你也可以要求商家退还货款。如果你无法获得商家信息，可以直接向12315投诉平台反映。你获得的赔偿数额将取决于你提交的证据，并由相关执法机关依法认定。
